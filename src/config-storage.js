export class ConfigStorage {
  constructor(state, env) {
    ...
    console.log('ConfigStorage: Initialized with VERSION_LIMIT:', this.VERSION_LIMIT);
  }
  ...
  async handlePost(request) {
    try {
      ...
      console.log(`Versions for new rule ${rule.id}:`, this.safeStringify(versions));

      await this.state.storage.put(`versions_${rule.id}`, JSON.stringify(versions));
  ...
  async getRule(ruleId) {
    try {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to retrieve rule', details: error.message }),
        {
          status: 500,
          headers: { 'Content-Type': 'application/json' },
        }
  ...
  async getRuleVersions(ruleId) {
    try {
      console.log(`ConfigStorage: Getting versions for rule with ID: ${ruleId}`);
      const versions = JSON.parse((await this.state.storage.get(`versions_${ruleId}`)) || '[]');
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to retrieve rule versions', details: error.message }),
        {
          status: 500,
          headers: { 'Content-Type': 'application/json' },
        }
    ...
    try {
      ...
      if (index !== -1) {
        ...
        const newRule = {
          ...
          fingerprint: {
            parameters: [
              ...(updatedRule.fingerprint?.parameters || oldRule.fingerprint.parameters),
            ],
          },
          initialMatch: {
            ...
            conditions: [
              ...(updatedRule.initialMatch?.conditions || oldRule.initialMatch.conditions),
            ],
            action: { ...oldRule.initialMatch.action, ...updatedRule.initialMatch?.action },
          },
          elseIfActions: [...(updatedRule.elseIfActions || oldRule.elseIfActions)],
        ...
        console.log('Saved rules (parsed):', this.safeStringify(JSON.parse(savedRules)));

        let versions = JSON.parse((await this.state.storage.get(`versions_${ruleId}`)) || '[]');
        ...
        const savedVersions = await this.state.storage.get(`versions_${ruleId}`);
        console.log('Saved versions (raw):', savedVersions);
        console.log('Saved versions (parsed):', this.safeStringify(JSON.parse(savedVersions)));

        console.log('Rule update complete');

        return new Response(JSON.stringify({ message: 'Rule updated', rule: newRule }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
  ...
  async reorderRules(updatedRules) {
    try {
      console.log('ConfigStorage: Reordering rules, received:', this.safeStringify(updatedRules));
      ...
      return new Response(JSON.stringify({ message: 'Rules reordered', rules: reorderedRules }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      });
    } catch (error) {
  ...
  async revertRule(ruleId, targetVersion) {
    try {
      console.log(`ConfigStorage: Reverting rule ${ruleId} to version ${targetVersion}`);
      let rules = await this.state.storage.get('rules');
      ...
      console.log('Target version data:', this.safeStringify(targetVersionData));
      ...
      return new Response(JSON.stringify({ message: 'Rule reverted', rule: revertedRule }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      });
    } catch (error) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to revert rule', details: error.message }),
        {
          status: 500,
          headers: { 'Content-Type': 'application/json' },
        }
  ...
  areValidRules(rules) {
    const isValid = Array.isArray(rules) && rules.every((rule) => this.isValidRuleStructure(rule));
